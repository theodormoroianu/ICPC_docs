...
struct Dinic
{
    ...
    bool bfs()
    {
        ...
        h[S] = 0;
        vector<int> q = {S};
        for (int it = 0; it < (int)q.size() && h[D] == -1; it++)
        {
            int nod = q[it];
...
int main()
{
    // https://codeforces.com/problemset/problem/704/D
    ...
    FlowWithMinRestrictions flow(5, 0, 4);

    flow.AddEdge(0, 1, 3, 4);
    flow.AddEdge(0, 2, 1, 2);
    flow.AddEdge(1, 3, 0, 3);
    flow.AddEdge(3, 2, 2, 5);
    flow.AddEdge(2, 4, 0, 3);
    flow.AddEdge(3, 4, 0, 10);

    bool feasible;
    int flow_value;

    vector<int> ans = flow.Solve(feasible, flow_value);

    assert(feasible && flow_value == 4);
    assert(ans == vector<int>({3, 1, 3, 2, 3, 1}));
