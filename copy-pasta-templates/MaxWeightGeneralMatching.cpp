...
struct WeightedGeneralMatching
{
    ...
    WeightedGeneralMatching(int n)
        : n(n), m(0), G(2 * n, vector<E>(2 * n)),
          lab(2 * n), match(2 * n), slack(2 * n), st(2 * n), pa(2 * n), S(2 * n), vis(2 * n),
          flo(2 * n), flo_from(2 * n, vector<int>(2 * n))
    {
        for (int x = 1; x <= n; ++x)
            for (int y = 1; y <= n; ++y)
                G[x][y] = E{x, y, 0};
    ...
    void AddEdgeIndexedFrom1(int x, int y, int w)
    {
        G[x][y].w = G[y][x].w = w;
    }
    ...
    void q_push(int x)
    {
        if (x <= n)
            ...
            for (int i = 0; i < (int)flo[x].size(); ++i)
                q_push(flo[x][i]);
    }
    ...
    pair<ll, int> Solve(vector<pair<int, int>> &ans)
    {
...
int main()
{
    ...
    WeightedGeneralMatching matching(6);

    matching.AddEdgeIndexedFrom1(1, 2, 10);
    matching.AddEdgeIndexedFrom1(1, 3, 5);
    matching.AddEdgeIndexedFrom1(2, 4, 6);
    matching.AddEdgeIndexedFrom1(5, 6, 10);

    vector<pair<int, int>> ans;
    auto [w, matched] = matching.Solve(ans);
    ...
    assert(matched == 3);
    assert(ans.size() == 3);
    vector<pair<int, int>> expected({{1, 3}, {2, 4}, {5, 6}});
    assert(ans == expected);
